...
class FaceAnalyzer:
    ...
    # def ExtractFrames(self, video_path, interval=0.10):
    #     self.reset_state()
    ...
    #     print(f"Total frame / Frame per second : {total_frame}/{fps}")
    #     if fps == 0:
    ...
    def ExtractFrames(self, video_path, interval=0.25):
        ...
        print(f"Total frames / Frame per second: {total_frames}/{fps}")
        if fps == 0:
            print("Error: FPS value is zero")
            return
...
def compute_average_reduced_features(video_folder, csv_file, known_faces_folder, region='us-east-1'):
    ...
    for index, row in df.iterrows():
        ...
        try:
            # if row['facial_feature'] == 0 or pd.isnull(row['facial_feature']):
            ...
            # else:
            #     video_features[index] = row['facial_feature']
            #     landmark_features[index] = row['landmarks']
            #     print(f"Facial feature and landmarks not updated: {video_features[index]} \n")
