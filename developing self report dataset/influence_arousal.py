...
# Load the dataset
file_path = 'Speakers_Arousal/Ross_a.xlsx'  # Change to the actual file path
...
for scene in data['scene_number'].unique():
    ...
    for i in range(1, len(scene_data)):
        prev_valency = normalize_valency(scene_data.loc[i - 1, 'arousal'])
        curr_valency = normalize_valency(scene_data.loc[i, 'arousal'])
...
for scene in data['scene_number'].unique():
    ...
    for i in range(1, len(scene_data)):
        ...
        if normalize_valency(scene_data.loc[i, 'arousal']) == normalize_valency(scene_data.loc[i - 1, 'valency']):
            sequence_length += 1
        else:
...
print(data[['scene_number', 'arousal', 'Sequence_Length']])
